...
contract DeployCertificateNFT is Script {
    function run() external returns (CertificateNFT certificateNFT) {
        ...
        string memory certificateBaseURI = vm.envOr("CERTIFICATE_BASE_URI", string("ipfs://certificate/"));
        address trustedSigner = vm.envAddress("MINT_TRUSTED_SIGNER_ADDRESS");

        vm.startBroadcast(deployerPrivateKey);
        certificateNFT = new CertificateNFT(name, symbol, certificateBaseURI, initialOwner, trustedSigner);
        vm.stopBroadcast();
    }
}
